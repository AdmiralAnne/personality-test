...
# api_key = os.getenv("helix-uakiVHJOWk5lLsw-EAeXawkOYVcNErFOLcO-2Md0aaE")  # Store your key in an environment variable for security
client = OpenAI(base_url="https://helixmind.online/v1", api_key='helix-uakiVHJOWk5lLsw-EAeXawkOYVcNErFOLcO-2Md0aaE') # do not steal this code
...
if st.button("Get My Top 20 Jobs"):
    if 'ocean_scores' in st.session_state and 'top_3_riasec_code' in st.session_state and 'domain_str' in st.session_state:
        ...
        generated_prompt_query = f"""
            ...
            if no domains are provided, stil.. organize the outputs by domain.
            Consider my education as well ( don't just stick to this info feel free to add non education related careers as well ): {education_background} - Ignore if no info is provided
